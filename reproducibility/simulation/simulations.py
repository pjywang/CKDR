...
from functions.cross_val import _get_trainer

# Parallel processing
from joblib import Parallel, delayed

from reproducibility.other_methods.rpy2setup import setup_r_environment
from reproducibility.realdata_prediction.prediction_comparison import _get_fold
...
def subsp_dist(A, B):
    ...
    return np.sqrt(max(sq_dist, 0.0)) # safe guard for numerical issues
...
def repeat_cv(n, p, Y_func, X_func, m=None, Y_noise=0.1,
              ...
              load=False, seed=None, solver="pgd"
               ):
    ...
    if load:
        ...
        result = ckdr_cv_parallel(X, Y, type_Y=type_Y,
                                  folds=5, verbose=True,
                                    dim_list=dim_list,
                                    n_jobs=min(50, njobs), seed=RS, solver=solver,
                                    refit=False)
    ...
    def process(i):
        ...
        P, obj_hist, model = trainer(X, Y, target_dim, type_Y, epsilon=epsilon, sigma=sigma, verbose=False, seed=RS)
...
def repeat_prediction(n, p, Y_func, X_func, m=None, Y_noise=0.1,
                      ...
                      load=True, load_folder="subsp_convergence", seed=None, solver="pgd"
                      ):
    """ 
    ...
    if load:
        # load the parameters from the previously trained model
        with open("./results/simulation/{}/param_op_{}_{}_{}{}.pickle".format(load_folder, n, Y_func.__name__, X_func.__name__, m), "rb") as f:
            params = pickle.load(f)
