...
def train_ckdr_pgd(
    ...
    sigma = np.median(pdist(X.detach().cpu().numpy())) if sigma is None else sigma
    ...
    if CKDR_data.K_Y is not None:
        G_Y = center(CKDR_data.K_Y).to(device=Y.device, dtype=Y.dtype)
    else:
    ...
    # TorchScript objective expects Python scalars; tensors inherit dtype from P and X.
    sigma = float(sigma)
    ...
    for iteration in range(max_iter):
        ...
        current_obj, gradient = T(P, X, Y, epsilon=epsilon, sigma=sigma, 
                                    gradient=True, G_Y=G_Y, Y_centered=Y_centered)
        ...
        while lr_k > armijo_min_lr:
            ...
            new_obj_candidate = T(P_candidate_projected, X, Y, 
                                    epsilon=epsilon, sigma=sigma, G_Y=G_Y, Y_centered=Y_centered).detach()
...
def project_CDR(A: torch.Tensor) -> torch.Tensor:
    ...
    z = torch.tensor(1.0, device=A.device, dtype=A.dtype)
    ...
    indices = torch.arange(1, A.shape[0] + 1, dtype=A.dtype, device=A.device).unsqueeze(1)
    ...
    projected_A = torch.maximum(A - theta.unsqueeze(0), torch.tensor(0.0, device=A.device, dtype=A.dtype))
